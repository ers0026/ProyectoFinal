...
<div class="col-md-10 offset-md-1 col-sm-10 offset-sm-1 bc-white">
	...
	<div class="registerAdmin" valign="middle">
		...
		<form action="ejecutarActualizarUsuarioAdmin.php" method="post">
				...
				Usuario<abbr title="El usuario nunca podrá quedar vacio, en caso de estar vacio no podremos guardar.">
					...
					</abbr><br> <input onkeyup="validar()" class="inputValidar" type="text" name="user" value="<?php echo $user?>"><br>
				...
				<!-- Contraseña <span class="info row" title="texto al pasar el raton"> -->
				Contraseña <abbr title="Introduce al menos una mayúscula, una minuscula y un dígito, texto entre 8 y 12 caracteres.">
					<i class="icon2 ion-md-information-circle col-lg-1"></i> 
				</abbr></span><br>
				<input type="password" name="pass" value="" placeholder="Introduce nueva contraseña" pattern="^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)[A-Za-z\d]{8,12}$"><br>
				...
				Repetir contraseña<br> <input type="password" name="repeatPass" value="" placeholder="Repite la nueva contraseña" pattern="^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)[A-Za-z\d]{8,12}$"><br>
				<br>
				<div class="row" style="margin:30px 0px 5px 0px">
					<button id="button" type="submit" class="btn btn-primary">Guardar</button>  

					<button type="button" class="btn btn-primary" style="margin-left:60px" onclick="location.href='index2.php'">Volver</button> 
        		</div>
